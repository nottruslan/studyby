-- Apply Order Chat migration (idempotent — safe to run once or retry).
-- Run this in Supabase Dashboard: SQL Editor → New query → paste → Run.
-- Fixes: "Could not find the table 'public.order_messages' in the schema cache"
...
-- 3. RLS policies (drop if exist so rerun is safe)
...
-- 4. Realtime (optional; skip if publication missing)
DO $$
BEGIN
...
-- 6. Storage RLS (drop if exist so rerun is safe)
